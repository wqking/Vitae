...
enum ZerocoinSpendStatus {
    ...
    ZVIT_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ...
    ZVIT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl = NULL, AvailableCoinsType coin_type = ALL_COINS, bool useIX = false, CAmount nFeePay = 0,
                           bool IsFundamentalNodePayment = false, bool fIncludeDelegated = false);
    ...
    bool CreateZVITOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);

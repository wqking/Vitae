...
/** Fees smaller than this (in uvit) are considered zero fee (for relaying, mining and transaction creation)
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_20_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zVIT spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!txIn.IsZerocoinPublicSpend())
                    return state.Invalid(error("%s: failed for tx %s, every input must be a zcpublicspend",
                            __func__, tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvit");
                ...
                if (!CheckPublicCoinSpendVersion(publicSpend.getVersion())) {
                    return state.Invalid(error("%s : Public Zerocoin spend version %d not accepted. must be version %d.",
                            __func__, publicSpend.getVersion(), CurrentPublicCoinSpendVersion()), REJECT_INVALID, "bad-txns-invalid-zvit");
                }
...
//        *pfMissingInputs = false;
    ...
    if (!CheckTransaction(tx, chainActive.Height() >= Params().GetConsensus().height_start_ZC, true, state))
        return error("AcceptableInputs: : CheckTransaction failed");
...
CAmount GetSeeSaw(int nHeight, int64_t blockValue){
        ...
        LogPrintf("Adjusting seesaw at height %d with %d masternodes (without drift: %d) at %ld\n", nHeight, nMasternodeCount, nMasternodeCount - Params().GetConsensus().nMasternodeCountDrift, GetTime());
...
{
    util::ThreadRename("vitae-scriptch");
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after height_start_ZC_PublicSpends so GetMasternodePayment is always 3 VIT.
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            ...
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(0, error("%s : Couldn't find fundamentalnode/budget payment", __func__),
                        REJECT_INVALID, "bad-cb-payee");
            }
        } else {
            LogPrintf("%s: Fundamentalnode/Budget payment checks skipped on sync\n", __func__);
        }
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nVitaeBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nVitaeBadBlockBits)) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> vitInputs;
        std::vector<CTxIn> zVITInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zVITInputs.push_back(stakeIn);
            }else{
                vitInputs.push_back(stakeIn);
            }
        }
        const bool hasVITInputs = !vitInputs.empty();
        const bool hasZVITInputs = !zVITInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasVITInputs) {
                    ...
                    for (const CTxIn& vitIn : vitInputs)
                        if(vitIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            if(hasZVITInputs) {
                for (const CTxIn& zVitInput : zVITInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zVitInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zvit");

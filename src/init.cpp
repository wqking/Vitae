...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-vitstake=<n>", strprintf(_("Enable or disable staking functionality for %s inputs (0-1, default: %u)"), CURRENCY_UNIT, 1));
    strUsage += HelpMessageOpt("-minstakesplit=<amt>", strprintf(_("Minimum positive amount (in VIT) allowed by GUI and RPC for the stake split threshold (default: %s)"),
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. VITAE Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. VITAE Core is probably already running."), strDataDir));
        ...
        do {
            ...
            try {
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zvit if already reindexed
                    RecalculateVITSupply(1, fReindexZerocoin);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzVITWallet(pwalletMain);

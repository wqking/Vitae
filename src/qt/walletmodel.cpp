...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool IsFundamentalNodePayment, bool fIncludeDelegations)
{
    CAmount total = 0;
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 VIT.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzVITReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzVITReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzVITReset.connect(boost::bind(NotifyzVITReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzVITReset.disconnect(boost::bind(NotifyzVITReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zvitTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}

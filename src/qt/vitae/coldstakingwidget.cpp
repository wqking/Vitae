    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your VITs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept VIT delegation / Delegate VIT"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select VIT outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 VIT" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
